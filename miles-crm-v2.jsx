...
const STYLE = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .auth-bg-circle:nth-child(1) { width: 600px; height: 600px; background: var(--gold); top: -200px; right: -100px; animation-delay: 0s; }
  ...
  .auth-feature-icon {
    ...
    background: rgba(232,160,32,0.1); border: 1px solid var(--border); flex-shrink: 0;
  }
  .auth-feature-title { font-size: 14px; font-weight: 600; margin-bottom: 2px; }
  .auth-feature-desc { font-size: 12px; color: var(--muted); }

  .auth-right {
    width: 480px; min-width: 480px; display: flex; align-items: center; justify-content: center;
    padding: 40px; position: relative; z-index: 2;
    background: rgba(12,12,26,0.6); border-left: 1px solid var(--border2);
    backdrop-filter: blur(20px);
  }

  .auth-panel { width: 100%; max-width: 400px; }
  .auth-tabs { display: flex; gap: 0; background: var(--surface); border-radius: var(--radius); padding: 4px; margin-bottom: 32px; border: 1px solid var(--border2); }
  .auth-tab {
  ...
  .form-input {
    ...
    font-family: 'Outfit'; font-size: 14px; outline: none;
    transition: all 0.2s;
  }
  ...
  .btn {
    ...
    cursor: pointer; border: none; font-family: 'Outfit'; display: inline-flex; align-items: center;
    justify-content: center; gap: 8px; transition: all 0.18s ease; letter-spacing: 0.2px;
  }
  .btn-full { width: 100%; }
  ...
  .btn-surface:hover { border-color: var(--border); background: var(--card2); }
  ...
  .social-btn {
    width: 100%; padding: 11px; border-radius: var(--radius-sm); background: var(--surface);
    border: 1px solid var(--border2); color: var(--text); font-family: 'Outfit';
    font-size: 13.5px; font-weight: 500; cursor: pointer; display: flex; align-items: center;
    justify-content: center; gap: 10px; transition: all 0.18s; margin-bottom: 10px;
  }
  .social-btn:hover { border-color: var(--border); background: var(--card2); }
  ...
  .auth-link { color: var(--gold); cursor: pointer; text-decoration: none; }
  .auth-link:hover { text-decoration: underline; }

  .input-error { border-color: rgba(232,64,64,0.5) !important; }
  .error-msg { font-size: 11.5px; color: var(--danger); margin-top: 4px; }
  .success-msg { font-size: 12px; color: var(--success); margin-top: 4px; }

  /* ── CRM ROOT ── */
  .crm-root { display: flex; height: 100vh; overflow: hidden; background: var(--bg); }

  /* ── SIDEBAR ── */
  .sidebar {
    width: 230px; min-width: 230px; background: var(--surface);
    border-right: 1px solid var(--border2); display: flex; flex-direction: column;
    position: relative; z-index: 10; transition: width 0.25s ease;
  }
  ...
  .nav-live { margin-left: auto; background: rgba(62,207,207,0.15); color: var(--teal); font-size: 9px; font-weight: 700; padding: 1px 6px; border-radius: 10px; letter-spacing: 0.5px; }

  .sidebar-user { padding: 14px 16px; border-top: 1px solid var(--border2); display: flex; align-items: center; gap: 10px; }
  .user-avatar {
    width: 34px; height: 34px; border-radius: 50%; background: linear-gradient(135deg, var(--gold), var(--purple));
  ...
  .header {
    ...
    background: rgba(12,12,26,0.9); position: sticky; top: 0; z-index: 5;
  ...
  .card-highlight { border-color: var(--border); background: linear-gradient(135deg, var(--card), rgba(232,160,32,0.04)); }
  ...
  .delta-dn { background: rgba(232,64,64,0.1); color: var(--danger); }
  ...
  .badge-purple { background: rgba(139,111,212,0.12); color: var(--purple); }
  ...
  .badge-pending { background: rgba(232,160,32,0.1); color: var(--warn); border: 1px solid rgba(232,160,32,0.2); }
  .badge-error { background: rgba(232,64,64,0.1); color: var(--danger); border: 1px solid rgba(232,64,64,0.2); }
  ...
  .modal {
    ...
    padding: 30px; width: 500px; max-width: 95vw; max-height: 88vh; overflow-y: auto;
  ...
  .connect-program-header {
    display: flex; align-items: center; gap: 14px; padding: 14px 16px;
    background: var(--surface); border-radius: var(--radius-sm); border: 1px solid var(--border2); margin-bottom: 20px;
  }
  .connect-logo { width: 44px; height: 44px; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 22px; }
  ...
  .success-icon { font-size: 48px; margin-bottom: 16px; animation: pop 0.4s ease; }
  ...
  .alert-info { background: rgba(75,168,255,0.06); border: 1px solid rgba(75,168,255,0.15); }
  .alert-success { background: rgba(31,184,106,0.06); border: 1px solid rgba(31,184,106,0.15); }
...
// ─────────────────────────────────────────────────────────────────────────────
// CATALOG DE PROGRAMAS
// ─────────────────────────────────────────────────────────────────────────────
const PROGRAMS_CATALOG = [
  {
    id: "latam", name: "LATAM Pass", type: "Aéreo", logo: "✈", color: "#E31837",
    bg: "rgba(227,24,55,0.1)", code: "pc-latam",
    desc: "Programa de fidelidade da LATAM Airlines. Acumule milhas em voos, cartões e parceiros.",
    partners: ["Banco do Brasil", "Bradesco", "Itaú", "Santander"],
    cpp: "R$ 0,022 - R$ 0,035", minTransfer: 1000, transferRate: "1:1",
    fields: ["login", "senha"], loginUrl: "https://www.latamairlines.com/br/pt/latam-pass"
  },
  {
    id: "smiles", name: "Smiles", type: "Aéreo", logo: "😊", color: "#FF6600",
    bg: "rgba(255,102,0,0.1)", code: "pc-smiles",
    desc: "Programa de milhagem da GOL Linhas Aéreas com vasta rede de acúmulo e resgate.",
    partners: ["Nubank", "Santander", "Bradescard", "Banese Card"],
    cpp: "R$ 0,018 - 0,030", minTransfer: 1000, transferRate: "1:1",
    fields: ["cpf", "senha"], loginUrl: "https://www.smiles.com.br"
  },
  {
    id: "livelo", name: "Livelo", type: "Banco", logo: "💎", color: "#7B2FBE",
    bg: "rgba(123,47,190,0.1)", code: "pc-livelo",
    desc: "Programa de pontos do Banco do Brasil e Bradesco. Transfira para vários aéreos.",
    partners: ["Banco do Brasil", "Bradesco", "Elo", "Visa"],
    cpp: "R$ 0,020 - 0,028", minTransfer: 1000, transferRate: "Variável",
    fields: ["cpf", "senha"], loginUrl: "https://www.livelo.com.br"
  },
  {
    id: "tudo", name: "TudoAzul", type: "Aéreo", logo: "💙", color: "#1170D4",
    bg: "rgba(17,112,212,0.1)", code: "pc-tudo",
    desc: "Programa de fidelidade da Azul Linhas Aéreas, referência em pontualidade.",
    partners: ["Itaú", "Caixa", "Sicredi", "Amex"],
    cpp: "R$ 0,020 - 0,032", minTransfer: 1000, transferRate: "1:1",
    fields: ["email", "senha"], loginUrl: "https://www.tudoazul.com.br"
  },
  {
    id: "miles-home", name: "Miles&More", type: "Aéreo Internacional", logo: "🌍", color: "#C8102E",
    bg: "rgba(200,16,46,0.1)", code: "pc-miles-home",
    desc: "Programa da Lufthansa Group. Ideal para voos internacionais e Star Alliance.",
    partners: ["Amex", "Diners", "Visa"],
    cpp: "R$ 0,028 - 0,045", minTransfer: 2000, transferRate: "1:1",
    fields: ["email", "senha"], loginUrl: "https://www.miles-and-more.com"
  },
  {
    id: "esfera", name: "Esfera", type: "Banco", logo: "⚡", color: "#E31837",
    bg: "rgba(227,24,55,0.1)", code: "pc-esfera",
    desc: "Programa do Santander. Acumule pontos e transfira para milhas aéreas.",
    partners: ["Santander", "Mastercard", "Visa"],
    cpp: "R$ 0,015 - 0,025", minTransfer: 2500, transferRate: "2.5:1",
    fields: ["cpf", "senha"], loginUrl: "https://www.esfera.com.br"
  },
  {
    id: "multiplus", name: "Multiplus", type: "Varejo", logo: "🛍", color: "#E8A020",
    bg: "rgba(232,160,32,0.1)", code: "pc-default",
    desc: "Programa de fidelidade multimarcas com mais de 300 empresas parceiras.",
    partners: ["TAM", "Porto Seguro", "Lojas Americanas"],
    cpp: "R$ 0,018 - 0,028", minTransfer: 1000, transferRate: "Variável",
    fields: ["cpf", "senha"], loginUrl: "https://www.multiplus.com.br"
  },
  {
    id: "itaucard", name: "Sempre Presente", type: "Banco", logo: "🧡", color: "#F5A623",
    bg: "rgba(245,166,35,0.1)", code: "pc-default",
    desc: "Programa de pontos do Itaú Unibanco para clientes dos cartões Itaucard.",
    partners: ["Itaú", "Visa", "Mastercard"],
    cpp: "R$ 0,020 - 0,030", minTransfer: 1000, transferRate: "1:1",
    fields: ["cpf", "senha"], loginUrl: "https://www.itau.com.br"
  },
];
...
// ─────────────────────────────────────────────────────────────────────────────
const MOCK_USERS = {
  "demo@milescrm.com": { password: "demo123", name: "Rafael Mendes", plan: "Premium", avatar: "RM" },
  "teste@teste.com": { password: "teste123", name: "Ana Beatriz", plan: "Pro", avatar: "AB" },
};

const CONNECTED_PROGRAMS_INIT = [
  { catalogId: "latam", account: "7654321", miles: 45230, expiry: "2025-06-15", lastSync: "2025-03-03T14:22:00", status: "connected" },
  { catalogId: "smiles", account: "1234567", miles: 128500, expiry: "2025-12-31", lastSync: "2025-03-03T14:20:00", status: "connected" },
  { catalogId: "livelo", account: "9988776", miles: 32100, expiry: "2026-03-20", lastSync: "2025-03-03T10:05:00", status: "connected" },
  { catalogId: "tudo", account: "5544332", miles: 8750, expiry: "2025-04-01", lastSync: "2025-03-02T18:30:00", status: "connected" },
];

const TRANSACTIONS_INIT = [
  { id: 1, date: "2025-03-01", description: "Compra Nubank Ultravioleta", program: "Smiles", type: "credit", amount: 4200 },
  { id: 2, date: "2025-02-28", description: "Voo GRU → SSA (GOL)", program: "Smiles", type: "debit", amount: -18000 },
  { id: 3, date: "2025-02-20", description: "Compra Cartão Itaú Personnalité", program: "LATAM Pass", type: "credit", amount: 5800 },
  { id: 4, date: "2025-02-15", description: "Transferência Livelo → Smiles (bônus 100%)", program: "Smiles", type: "transfer", amount: 10000 },
  { id: 5, date: "2025-02-10", description: "Hotel Marriott Bonvoy", program: "Livelo", type: "credit", amount: 2400 },
  { id: 6, date: "2025-01-28", description: "Compra iFood", program: "TudoAzul", type: "credit", amount: 320 },
  { id: 7, date: "2025-01-20", description: "Upgrade de cabine GRU-GIG", program: "LATAM Pass", type: "debit", amount: -8000 },
  { id: 8, date: "2025-01-15", description: "Expiração parcial", program: "TudoAzul", type: "debit", amount: -2500 },
  { id: 9, date: "2025-01-05", description: "Compra Americanas.com", program: "Livelo", type: "credit", amount: 1200 },
];
...
const AI_CHIPS = [
  "Milhas prestes a vencer?", "Melhor programa para Europa?",
  "Promoções de transferência ativas?", "Estratégia para 100k milhas?",
  "CPP atual do Smiles?", "Vale comprar milhas agora?",
];
...
Carteira do usuário:
${connected.map(p => { const c = getCatalog(p.catalogId); return `- ${c.name}: ${fmt(p.miles)} milhas | Vence: ${fmtDate(p.expiry)} (${daysUntil(p.expiry)} dias)`; }).join("\n")}
Total: ${fmt(total)} milhas ≈ ${fmtBRL(total * 0.025)}
Responda em português, seja objetivo e prático. Use **negrito** para destaques. Forneça estratégias baseadas nos dados reais do usuário.`;
...
function AuthScreen({ onLogin }) {
  ...
  const handleLogin = async () => {
    ...
    const u = MOCK_USERS[form.email.toLowerCase()];
    if (!u || u.password !== form.password) {
      setErr({ password: "E-mail ou senha inválidos" });
  ...
  const handleRegister = async () => {
    ...
    setLoading(false); setRegDone(true);
    setTimeout(() => { setTab("login"); setRegDone(false); setForm(p => ({ ...p, password: "", confirm: "" })); }, 2200);
  };

  const demoLogin = () => { setForm({ email: "demo@milescrm.com", password: "demo123", name: "", confirm: "" }); setErr({}); };
    ...
    <div className="auth-root">
      ...
      <div className="auth-left">
        <div className="auth-brand">
          <div className="auth-logo">MilesCRM</div>
          <div className="auth-logo-sub">Gestão de Milhas & Pontos</div>
        </div>
        <div className="auth-headline">Gerencie suas<br /><em>milhas</em> com<br />inteligência</div>
        <div className="auth-desc">Conecte todos os seus programas de fidelidade em um único painel. Nunca mais perca milhas por vencimento ou deixe de aproveitar promoções.</div>
        <div className="auth-features">
          {[
            { icon: "🔗", title: "8 programas integrados", desc: "LATAM, Smiles, Livelo, TudoAzul e mais" },
            { icon: "🤖", title: "Assistente IA com busca real", desc: "Respostas com dados em tempo real da web" },
            { icon: "🔔", title: "Alertas de vencimento", desc: "Notificações para nunca perder milhas" },
          ].map((f, i) => (
        ...
        <div className="auth-panel">
          ...
          {tab === "login" ? (
            <>
              ...
              <div className="auth-divider">ou acesse rapidamente</div>
              <button className="social-btn" onClick={demoLogin}>
                <span>🎮</span> Usar conta demo (demo@milescrm.com)
              </button>
              <div className="auth-footer">
                Não tem conta? <span className="auth-link" onClick={() => setTab("register")}>Cadastre-se grátis</span>
              </div>
            </>
          ) : (
            <>
              ...
              {regDone ? (
                <div className="success-box" style={{ padding: "20px 0" }}>
                  ...
                  <div className="success-desc">Redirecionando para o login...</div>
                </div>
              ) : (
                <>
                  <div className="form-group">
                    <label className="form-label">Nome completo</label>
                    <input className={`form-input ${err.name ? "input-error" : ""}`} placeholder="João Silva" value={form.name} onChange={e => set("name", e.target.value)} />
                    {err.name && <div className="error-msg">{err.name}</div>}
                  </div>
                  <div className="form-group">
                    <label className="form-label">E-mail</label>
                    <input className={`form-input ${err.email ? "input-error" : ""}`} type="email" placeholder="seu@email.com" value={form.email} onChange={e => set("email", e.target.value)} />
                    {err.email && <div className="error-msg">{err.email}</div>}
                  </div>
                  <div className="form-row">
                    <div className="form-group">
                      <label className="form-label">Senha</label>
                      <input className={`form-input ${err.password ? "input-error" : ""}`} type="password" placeholder="••••••••" value={form.password} onChange={e => set("password", e.target.value)} />
                      {err.password && <div className="error-msg">{err.password}</div>}
                    </div>
                    <div className="form-group">
                      <label className="form-label">Confirmar</label>
                      <input className={`form-input ${err.confirm ? "input-error" : ""}`} type="password" placeholder="••••••••" value={form.confirm} onChange={e => set("confirm", e.target.value)} />
                      {err.confirm && <div className="error-msg">{err.confirm}</div>}
                    </div>
                  </div>
                  <button className="btn btn-gold btn-full" onClick={handleRegister} disabled={loading}>
                    {loading ? <><div className="spinner" style={{ borderTopColor: "#000" }} /> Criando...</> : "Criar conta grátis →"}
                  </button>
                  <div className="auth-footer" style={{ marginTop: 16 }}>
                    Ao criar conta você concorda com os <span className="auth-link">Termos de Uso</span>
                  </div>
                </>
              )}
            </>
...
// ── CONNECT MODAL ─────────────────────────────────────────────────────────────
function ConnectModal({ program, onClose, onConnect }) {
  ...
  const handleConnect = async () => {
    ...
    onConnect({
      ...
      account: form[program.fields[0]] || "******",
      miles: m,
      expiry: new Date(Date.now() + 1000 * 60 * 60 * 24 * 365).toISOString().split("T")[0],
  ...
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        <div className="steps">
          {[1, 2, 3].map(s => <div key={s} className={`step ${step === s ? "active" : step > s ? "done" : ""}`} />)}
        </div>
          ...
          <>
            ...
            <div className="connect-program-header">
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: "var(--muted)" }}>{program.type}</div>
              </div>
              <div style={{ marginLeft: "auto" }}><span className="badge badge-muted">{program.transferRate}</span></div>
            </div>
            <div className="info-box">
              <strong>🔐 Segurança:</strong> Suas credenciais são criptografadas e usadas apenas para leitura do saldo. Nunca realizamos transações em seu nome.<br /><br />
              <strong>📊 O que será importado:</strong> Saldo atual, data de vencimento e histórico de transações dos últimos 12 meses.<br /><br />
              <strong>🔄 Sincronização:</strong> Automática a cada 24h ou manual a qualquer momento.
            </div>
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 12, color: "var(--muted)", marginBottom: 8, fontWeight: 500 }}>Parceiros aceitos:</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {program.partners.map(p => <span key={p} className="badge badge-muted">{p}</span>)}
              </div>
            </div>
            <div className="modal-actions">
              <button className="btn btn-ghost" onClick={onClose}>Cancelar</button>
              <button className="btn btn-gold" onClick={() => setStep(2)}>Continuar →</button>
            </div>
          </>
        )}
          ...
          <>
            ...
            <div className="connect-program-header">
              ...
              <div>
                ...
                <a href={program.loginUrl} target="_blank" rel="noreferrer" style={{ fontSize: 11, color: "var(--teal)", textDecoration: "none" }}>Criar conta no site →</a>
              </div>
            </div>
            {program.fields.map(f => (
              <div className="form-group" key={f}>
                <label className="form-label">{fieldLabel[f] || f}</label>
                <input
                  className="form-input"
                  type={f === "senha" ? "password" : f === "cpf" ? "text" : "text"}
                  placeholder={f === "cpf" ? "000.000.000-00" : f === "senha" ? "••••••••" : "seu@email.com"}
                  value={form[f] || ""}
                  onChange={e => set(f, e.target.value)}
                />
              </div>
            ))}
            <div className="info-box" style={{ fontSize: 11.5 }}>
              💡 <strong>Dica:</strong> Para testes, use qualquer valor — a integração é simulada nesta demonstração.
            </div>
            <div className="modal-actions">
              <button className="btn btn-ghost" onClick={() => setStep(1)}>← Voltar</button>
              <button className="btn btn-gold" onClick={handleConnect} disabled={loading}>
                {loading ? <><div className="spinner" style={{ borderTopColor: "#000", width: 14, height: 14 }} /> Conectando...</> : "🔗 Conectar"}
              </button>
            </div>
          </>
        )}
        ...
        {step === 3 && (
          <div className="success-box">
            <div className="success-icon">✅</div>
            <div className="success-title">{program.name} conectado!</div>
            <div className="success-desc">Saldo importado com sucesso</div>
            <div className="success-miles">{fmt(miles)}</div>
            <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 4, marginBottom: 20 }}>milhas encontradas</div>
            <button className="btn btn-gold" onClick={onClose}>Concluído ✓</button>
          </div>
        )}
      </div>
    </div>
...
// ── INTEGRATIONS VIEW ─────────────────────────────────────────────────────────
function Integrations({ connected, onConnect, onDisconnect }) {
  const [connecting, setConnecting] = useState(null);
  const connectedIds = new Set(connected.map(p => p.catalogId));

  return (
    <div className="animate-fade">
      <div style={{ marginBottom: 24 }}>
        <div className="section-title" style={{ marginBottom: 6 }}>Programas <span>Disponíveis</span></div>
        <p style={{ fontSize: 13, color: "var(--muted)" }}>Conecte seus programas para importar saldo e manter tudo sincronizado automaticamente.</p>
      </div>

      {connected.length > 0 && (
        <>
          <div style={{ fontSize: 12, color: "var(--success)", fontWeight: 600, marginBottom: 12, display: "flex", alignItems: "center", gap: 8 }}>
            <span className="status-dot dot-green" /> {connected.length} programa{connected.length > 1 ? "s" : ""} conectado{connected.length > 1 ? "s" : ""}
          </div>
      ...
      <div className="integration-grid">
        {PROGRAMS_CATALOG.map(prog => {
          const conn = connected.find(c => c.catalogId === prog.id);
          const isConn = !!conn;
          return (
            <div key={prog.id} className={`integration-card ${isConn ? "connected" : ""}`}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                <div className="int-logo-wrap" style={{ background: prog.bg }}>
                  <span style={{ fontSize: 26 }}>{prog.logo}</span>
                </div>
                {isConn
                  ? <span className="badge badge-connected">✓ Conectado</span>
                  : <span className="badge badge-muted">{prog.type}</span>
                }
              </div>
                ...
                <>
                  <div className="int-stats">
                    <div className="int-stat">
                      <div className="int-stat-val">{fmt(conn.miles)}</div>
                      <div className="int-stat-label">Milhas</div>
                    </div>
                    <div className="int-stat">
                      <div className="int-stat-val">{daysUntil(conn.expiry)}d</div>
                      <div className="int-stat-label">Vencimento</div>
                    </div>
                    <div className="int-stat">
                      <div className="int-stat-val">{fmtDateTime(conn.lastSync).split(",")[0]}</div>
                      <div className="int-stat-label">Última sync</div>
                    </div>
                  </div>
                  <div style={{ marginBottom: 12 }}>
                    <div className="sync-pulse">
                      <div className="pulse-dot" /> Sincronização automática ativa
                    </div>
                  </div>
                  <div className="int-actions">
                    <button className="btn btn-teal btn-sm" style={{ flex: 1 }}>↻ Sincronizar agora</button>
                    <button className="btn btn-danger btn-sm" onClick={() => onDisconnect(prog.id)}>Desconectar</button>
                  </div>
                </>
              ) : (
                <>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
                    <span style={{ fontSize: 12, color: "var(--muted)" }}>CPP: <span style={{ color: "var(--text)" }}>{prog.cpp}</span></span>
                    <span style={{ fontSize: 12, color: "var(--muted)" }}>Transferência: <span style={{ color: "var(--text)" }}>{prog.transferRate}</span></span>
                  </div>
                  <div className="int-actions">
                    <button className="btn btn-gold btn-sm" style={{ flex: 1 }} onClick={() => setConnecting(prog)}>
                      🔗 Conectar
                    </button>
                    <a href={prog.loginUrl} target="_blank" rel="noreferrer" className="btn btn-ghost btn-sm">Site</a>
                  </div>
                </>
              )}
            </div>
      ...
      {connecting && (
        <ConnectModal
          ...
          onConnect={data => { onConnect(data); setConnecting(null); }}
        />
      )}
    </div>
  );
...
// ── DASHBOARD ─────────────────────────────────────────────────────────────────
function Dashboard({ connected, transactions }) {
  const total = connected.reduce((s, p) => s + p.miles, 0);
  const expiring = connected.filter(p => daysUntil(p.expiry) < 90).reduce((s, p) => s + p.miles, 0);
  const pieData = connected.map(p => { const c = getCatalog(p.catalogId); return { name: c.name, value: p.miles, color: c.color }; });
  const barData = connected.map(p => { const c = getCatalog(p.catalogId); return { name: c.name?.split(" ")[0], miles: p.miles, fill: c.color }; });

  return (
    <div className="animate-fade">
      <div className="grid-4">
        {[
          { label: "Total de Milhas", val: fmt(total), color: "var(--gold)", sub: `${connected.length} programas`, delta: "↑ +8.3% vs mês anterior", dc: "up" },
          ...
          <div key={i} className="card" style={i === 0 ? { borderColor: "var(--border)" } : {}}>
            <div className="metric-label">{m.label}</div>
            <div className="metric-value" style={{ color: m.color, fontSize: 24 }}>{m.val}</div>
            <div className="metric-sub">{m.sub}</div>
            <div className={`delta delta-${m.dc}`}>{m.delta}</div>
          </div>
        ))}
      </div>

      {connected.filter(p => daysUntil(p.expiry) < 30).map(p => {
        const c = getCatalog(p.catalogId);
        return (
          <div key={p.catalogId} className="alert alert-danger">
            🚨 <div><strong style={{ color: "var(--danger)" }}>{c.name}</strong> — {fmt(p.miles)} milhas vencem em <strong>{daysUntil(p.expiry)} dias</strong> ({fmtDate(p.expiry)}). Resgate ou transfira urgentemente!</div>
          </div>
        );
      })}
      {connected.filter(p => daysUntil(p.expiry) >= 30 && daysUntil(p.expiry) < 90).map(p => {
        const c = getCatalog(p.catalogId);
        return (
          <div key={p.catalogId} className="alert alert-warn">
            ⚠️ <div><strong style={{ color: "var(--warn)" }}>{c.name}</strong> — {fmt(p.miles)} milhas vencem em {daysUntil(p.expiry)} dias. Planeje um resgate em breve.</div>
          </div>
        );
      })}
            ...
            <ResponsiveContainer width={150} height={150}>
              <PieChart>
                <Pie data={pieData} cx="50%" cy="50%" innerRadius={42} outerRadius={68} dataKey="value" stroke="none">
                  {pieData.map((e, i) => <Cell key={i} fill={e.color} opacity={0.82} />)}
                </Pie>
                <Tooltip contentStyle={{ background: "#161628", border: "1px solid rgba(232,160,32,0.2)", borderRadius: 8, fontSize: 12 }} formatter={v => [fmt(v), "milhas"]} />
              </PieChart>
            </ResponsiveContainer>
            <div style={{ flex: 1 }}>
              {pieData.map((p, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 9 }}>
                  ...
                  <span style={{ fontSize: 12, color: "var(--muted)", flex: 1 }}>{p.name}</span>
                  <span style={{ fontSize: 12, fontFamily: "JetBrains Mono" }}>{fmt(p.value)}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="card">
        <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 16, color: "var(--muted)" }}>Comparativo de Saldo</div>
        <ResponsiveContainer width="100%" height={170}>
          <BarChart data={barData} barSize={36}>
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" vertical={false} />
            <XAxis dataKey="name" stroke="#555577" tick={{ fontSize: 12 }} />
            <YAxis stroke="#555577" tick={{ fontSize: 11 }} tickFormatter={v => fmt(v / 1000) + "k"} />
            <Tooltip contentStyle={{ background: "#161628", border: "1px solid rgba(232,160,32,0.2)", borderRadius: 8, fontSize: 12 }} formatter={v => [fmt(v), "milhas"]} />
...
function Programs({ connected, onGoIntegrations }) {
  if (connected.length === 0) {
    return (
      <div className="animate-fade empty">
        <div className="empty-icon">🔗</div>
        <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 8 }}>Nenhum programa conectado</div>
        <div className="empty-text" style={{ marginBottom: 20 }}>Vá em Integrações para conectar seus programas de milhas</div>
        <button className="btn btn-gold" onClick={onGoIntegrations}>Conectar programas →</button>
      </div>
    );
  }
  ...
  return (
    <div className="animate-fade">
      <div className="section-head">
        <div className="section-title">Meus <span>Programas</span></div>
        <button className="btn btn-surface btn-sm" onClick={onGoIntegrations}>+ Conectar mais</button>
      </div>
      <div className="program-grid">
        {connected.map(p => {
          ...
          return (
            <div key={p.catalogId} className={`program-card ${c.code || "pc-default"}`}>
              <div className="prog-head">
                <div>
                  <div className="prog-name">{c.name}</div>
                  <div className="prog-account">#{p.account}</div>
                </div>
                <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                  <span className="badge badge-connected" style={{ fontSize: 10 }}>✓ Sync</span>
                  <div className="prog-logo" style={{ background: c.bg, color: c.color }}>{c.logo}</div>
                </div>
              </div>
              <div>
                <div className="prog-miles-label">Saldo atual</div>
                <div className="prog-miles">{fmt(p.miles)}</div>
                <div className="prog-brl">≈ {fmtBRL(p.miles * 0.025)}</div>
              </div>
              {p.expiry && (
                <div className={`prog-expiry ${expiryClass(days)}`}>
                  {expiryIcon(days)} Vence {fmtDate(p.expiry)} · {days} dias
                </div>
              )}
              <div className="prog-foot">
                <div style={{ flex: 1, fontSize: 11, color: "var(--muted)" }}>
                  Sync: {fmtDateTime(p.lastSync).split(",")[0]}
                </div>
                <button className="btn btn-teal btn-xs">↻</button>
              </div>
            </div>
          );
        })}
...
// ── TRANSACTIONS ──────────────────────────────────────────────────────────────
function Transactions({ transactions, connected, onAdd }) {
  const [filter, setFilter] = useState("todos");
  const [filterProg, setFilterProg] = useState("todos");
  const [showModal, setShowModal] = useState(false);
  const [form, setForm] = useState({ date: new Date().toISOString().split("T")[0], description: "", program: connected[0] ? getCatalog(connected[0].catalogId).name : "", type: "credit", amount: "" });

  const filtered = transactions
    .filter(t => filter === "todos" || t.type === filter)
    .filter(t => filterProg === "todos" || t.program === filterProg)
  ...
  const submit = () => {
    ...
    onAdd({ ...form, id: Date.now(), amount: form.type === "debit" ? -amt : amt });
    setShowModal(false);
  ...
  return (
    <div className="animate-fade">
      <div className="section-head">
        <div className="section-title">Extrato de <span>Transações</span></div>
        <button className="btn btn-gold btn-sm" onClick={() => setShowModal(true)}>+ Nova transação</button>
      </div>
      <div className="filter-row">
        ...
        <select className="filter-select" value={filterProg} onChange={e => setFilterProg(e.target.value)}>
          ...
          {connected.map(p => { const c = getCatalog(p.catalogId); return <option key={p.catalogId}>{c.name}</option>; })}
        </select>
      </div>
      <div className="card" style={{ padding: 0, overflow: "hidden" }}>
        <div className="table-wrap">
          <table>
            ...
            <tbody>
              {filtered.map(t => {
                const [bc, label] = typeMap[t.type] || ["badge-muted", t.type];
                return (
                  <tr key={t.id}>
                    <td style={{ color: "var(--muted)", fontSize: 12 }}>{fmtDate(t.date)}</td>
                    <td>{t.description}</td>
                    <td><span className="badge badge-gold" style={{ fontSize: 10 }}>{t.program}</span></td>
                    <td><span className={`badge ${bc}`}>{label}</span></td>
                    <td style={{ textAlign: "right" }}>
                      <span className={t.type === "credit" ? "tx-credit" : t.type === "debit" ? "tx-debit" : "tx-transfer"}>
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowModal(false)}>
          <div className="modal">
            ...
            <div className="form-group">
              ...
              <input className="form-input" placeholder="ex: Compra Cartão Nubank" value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} />
            </div>
            <div className="form-row">
              <div className="form-group">
                <label className="form-label">Programa</label>
                <select className="form-input filter-select" value={form.program} onChange={e => setForm(p => ({ ...p, program: e.target.value }))}>
                  {connected.map(p => { const c = getCatalog(p.catalogId); return <option key={p.catalogId}>{c.name}</option>; })}
                </select>
              </div>
              <div className="form-group">
                <label className="form-label">Tipo</label>
                <select className="form-input filter-select" value={form.type} onChange={e => setForm(p => ({ ...p, type: e.target.value }))}>
            ...
            <div className="form-row">
              <div className="form-group">
                <label className="form-label">Milhas</label>
            ...
            <div className="modal-actions">
              ...
              <button className="btn btn-gold" onClick={submit}>Registrar →</button>
            </div>
          </div>
        </div>
      )}
...
function AIAdvisor({ connected }) {
  ...
  const [messages, setMessages] = useState([{
    role: "assistant",
    content: `Olá! Sou seu **Assistente de Milhas** com busca em tempo real. 🌟\n\nTenho acesso à sua carteira com **${fmt(total)} milhas** em ${connected.length} programas conectados.\n\nPosso pesquisar promoções ativas, calcular estratégias e responder sobre qualquer programa. O que deseja saber?`
  }]);
  ...
  const send = async (text) => {
    ...
    setLoading(true);
    try {
      const reply = await askClaude(newMsgs.filter(m => typeof m.content === "string"), connected);
    ...
    <div className="ai-panel animate-fade">
      <div className="ai-messages">
        {messages.map((m, i) => (
          <div key={i} className={`msg ${m.role}`}>
            ...
            <div className="msg-bubble">{formatAI(m.content)}</div>
          </div>
        ))}
        {loading && (
          <div className="msg">
            <div className="msg-avatar">✦</div>
            <div className="msg-bubble" style={{ padding: "14px 18px" }}><Dots /></div>
          </div>
        )}
        <div ref={bottomRef} />
      </div>
      <div className="ai-input-area">
        <div className="ai-chips">
          {AI_CHIPS.map((c, i) => <div key={i} className="chip" onClick={() => send(c)}>{c}</div>)}
        </div>
  ...
  return (
    <div className="animate-fade">
      ...
      {sorted.map(p => {
        const c = getCatalog(p.catalogId);
        const days = daysUntil(p.expiry);
        const pct = Math.max(5, Math.min(100, (1 - days / 365) * 100));
        const fc = days < 30 ? "var(--danger)" : days < 90 ? "var(--warn)" : "var(--success)";
        return (
          <div key={p.catalogId} className="card" style={{ marginBottom: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
              <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                <div style={{ width: 40, height: 40, borderRadius: 10, background: c.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>{c.logo}</div>
                <div>
                  <div style={{ fontWeight: 700, fontSize: 15 }}>{c.name}</div>
                  <div style={{ fontSize: 11, color: "var(--muted)" }}>#{p.account}</div>
                </div>
              </div>
              <div style={{ textAlign: "right" }}>
                <div style={{ fontFamily: "JetBrains Mono", fontSize: 20, color: "var(--gold)" }}>{fmt(p.miles)}</div>
                <div style={{ fontSize: 11, color: "var(--muted)" }}>milhas</div>
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
              <span className={`badge ${days < 30 ? "badge-danger" : days < 90 ? "badge-warn" : "badge-success"}`}>{expiryIcon(days)} {days} dias restantes</span>
              <span style={{ fontSize: 12, color: "var(--muted)" }}>Vence em {fmtDate(p.expiry)}</span>
            </div>
            ...
            {days < 90 && (
              <div className="alert alert-warn" style={{ marginTop: 12, marginBottom: 0 }}>
                💡 {days < 30 ? `Resgate urgente! Use no site ${c.name} ou transfira para programa compatível.` : `Planeje um resgate parcial para garantir que suas milhas não expirem.`}
              </div>
            )}
          </div>
        );
...
export default function App() {
  ...
  const [connected, setConnected] = useState(CONNECTED_PROGRAMS_INIT);
  ...
  const handleConnect = (data) => {
    setConnected(prev => {
      const exists = prev.findIndex(p => p.catalogId === data.catalogId);
      if (exists >= 0) { const n = [...prev]; n[exists] = data; return n; }
      return [...prev, data];
    });
  };

  const handleDisconnect = (catalogId) => {
    setConnected(prev => prev.filter(p => p.catalogId !== catalogId));
  };
      ...
      <div className="crm-root">
        ...
        <aside className="sidebar">
          <div className="sidebar-logo">
            <div>
              <div className="sidebar-logo-mark">MilesCRM</div>
              <div className="sidebar-logo-sub">Gestão Premium</div>
            </div>
          </div>
          ...
          <div className="sidebar-user">
            ...
            <div className="logout-btn" onClick={() => setUser(null)} title="Sair">⏻</div>
          </div>
        </aside>
        ...
        <main className="main">
          <header className="header">
            <div>
              <div className="header-title">{HEADER_MAP[view]?.title}</div>
              <div className="header-sub">{HEADER_MAP[view]?.sub}</div>
            </div>
            <div className="header-actions">
              {view === "ai"
                ? <span className="badge badge-teal">● Online · Web Search ativo</span>
                : <span className="badge badge-gold">✦ {fmt(total)} milhas</span>
              }
            </div>
          </header>

          <div style={isAI ? { padding: 0, display: "flex", flexDirection: "column", flex: 1 } : { padding: "24px 28px", flex: 1 }}>
            {view === "dashboard" && <Dashboard connected={connected} transactions={transactions} />}
            {view === "integrations" && <Integrations connected={connected} onConnect={handleConnect} onDisconnect={handleDisconnect} />}
            {view === "programs" && <Programs connected={connected} onGoIntegrations={() => setView("integrations")} />}
            {view === "transactions" && <Transactions transactions={transactions} connected={connected} onAdd={t => setTransactions(p => [t, ...p])} />}
